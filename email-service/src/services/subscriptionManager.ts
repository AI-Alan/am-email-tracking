...
async function createSubscription(): Promise<string | null> {
    try {
...
export async function initializeSubscription(): Promise<void> {
    console.log("🔔 Initializing webhook subscription...");
    ...
    if (!currentSubscriptionId) {
        console.error("⚠️ Failed to initialize subscription. Reply tracking may not work.");
        return;
    }
